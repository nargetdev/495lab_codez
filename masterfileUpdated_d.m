...
i=1;
for i=1:2
    filename=['decouple' num2str(i)];
    Data=importdata(filename);
    Data = Data.data;
    decouple_time(:,i)=Data(:,1);
    decouple_voltage(:,i)=Data(:,2);
    decouple_current(:,i)=Data(:,3);
    decouple_distaltach(:,i)=Data(:,4);
    decouple_motortach(:,i)=Data(:,5);
end
...
for i=[1:5]
    ...
    tau_lefte(1,i)=log(lefteraser_time(pkslocs(2),i))/log(lefteraser_time(pkslocs(1),i));
...
%     r_righte(1,i)=mean(r_each(:,i));
    ...
    tau_righte(1,i)=log(righteraser_time(pkslocs(2),i))/log(righteraser_time(pkslocs(1),i));
...
for i=[1:2]
    ...
    decouple_fit=polyfit(decouple_time(4001:12001,i),decouple_distaltach(4001:12001,i),1);
    %slope=onoff_fit(1);
    bla5=input('do you want decouple plot? 1=yes \n');
    if bla5==1
        figure
        plot(decouple_time(:,i), decouple_distaltach(:,i),decouple_time(:,i),decouple_fit(1).*decouple_time(:,i)+decouple_fit(2))
        xlabel('time(s)')  
        ylabel('distaltach') 
    else
        break
...
%     B_tl=2*zeta_x_wn_righte*(J_m+J_f+J_s);
    ...
    B_m=B_tl-B_tr;
    %B_b=0.5*(B_onoff-B_m);
    B_b=decouple_fit(1)*J_onoff*-1/2;
    B_s=(B_tr-B_b);
